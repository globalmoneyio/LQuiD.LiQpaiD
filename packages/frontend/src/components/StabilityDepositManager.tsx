  ...
  const actions = [
    ...(difference
      ? difference.positive
        ? ([
            [
              `Deposit ${difference.absoluteValue!.prettify()} LQD${
                originalDeposit.pendingCollateralGain.nonZero
                  ? ` & withdraw ${originalDeposit.pendingCollateralGain.prettify(4)} NEAR`
                  : ""
              }`,
              liquity.depositQuiInStabilityPool.bind(liquity, difference.absoluteValue!),
              [[quiBalance.gte(difference.absoluteValue!), "You don't have enough LQD"]]
            ]
          ] as const)
        : ([
            [
              `Withdraw ${difference.absoluteValue!.prettify()} LQD${
                originalDeposit.pendingCollateralGain.nonZero
                  ? ` & ${originalDeposit.pendingCollateralGain.prettify(4)} NEAR`
                  : ""
              }`,
      ...
      : ([
          ...
          ...(!trove.isEmpty
            ? ([
                [
                  `Transfer ${originalDeposit.pendingCollateralGain.prettify(4)} NEAR to Trove`,
