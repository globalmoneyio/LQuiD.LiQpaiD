  ...
  const [actionName, send, extraRequirements] = original.isEmpty
        ...
        edited.debt.nonZero
          ? ([
              [
                ...
                "Can't borrow LQD during recovery mode"
              ],
              [
    ...
    ? ([
        ...
        [
          ...
          [quiBalance.gte(original.debt), "You don't have enough LQD"]
        ]
      ] as const)
    : ([
        collateralDifference && debtDifference
          ? collateralDifference.positive && debtDifference.positive
            ? `Deposit ${collateralDifference.absoluteValue!.prettify()} NEAR & ` +
              `borrow ${debtDifference.absoluteValue!.prettify()} LQD`
            : collateralDifference.negative && debtDifference.negative
            ? `Repay ${debtDifference.absoluteValue!.prettify()} LQD & ` +
              `withdraw ${collateralDifference.absoluteValue!.prettify()} NEAR`
          ...
          ? `${collateralDifference.positive ? "Deposit" : "Withdraw"} ` +
            `${collateralDifference.absoluteValue!.prettify()} NEAR`
          : `${debtDifference!.positive ? "Borrow" : "Repay"} ` +
            `${debtDifference!.absoluteValue!.prettify()} LQD`,
        ...
        [
          ...(collateralDifference?.negative
            ? ([
                [
                  !total.collateralRatioIsBelowCritical(price),
                  "Can't withdraw NEAR during recovery mode"
                ]
              ] as const)
            : []),
          ...(debtDifference?.positive
          ...
          ...(debtDifference?.negative
            ? ([
                [quiBalance.gte(debtDifference.absoluteValue!), "You don't have enough LQD"]
              ] as const)
            : [])
        ]
      ] as const);
...
export const TroveManager: React.FC<TroveManagerProps> = ({
  ...
  return (
    <>
      <TroveEditor
        title={original.isEmpty ? "Open a new Trove" : "My Trove"}
        {...{
          original,
          edited,
          setEdited,
